...
model_list2 <- list(
      ...
      slc <- lmer(tot_lfct.log ~ hrs_abv25ds + avg_hrs1422_wet_t_t1.log + H.2005 + twi15m.log + (1|plotid) + (1|sample_year), data = oakplots.sub),
      
      wet_hours_1422 <- lmer(avg_hrs1422_wet_t_t1.log ~ twi15m.log + (1|plotid) + (1|sample_year), data = oakplots.sub),
...
model_list2.scl <- list(
      ...
      slc <- lmer(tot_lfct.log.scl ~ hrs_abv25ds.scl + avg_hrs1422_wet_t_t1.log.scl + shannons2005.scl + twi15m.log.scl + (1|plotid) + (1|sample_year), data = oakplots.sub),
      
      wet_hours_1422 <- lmer(avg_hrs1422_wet_t_t1.log.scl ~ twi15m.log.scl + (1|plotid) + (1|sample_year), data = oakplots.sub),
...
## Temperature Figures
```{r temperature figures, echo=F, warning=F, eval=F}
library(ggplot2)
library(scales)
...
rainfall_figure +
      ...
      geom_hline(aes(yintercept = mean(rain2d_total)),
                 colour = "red") +
      xlab("Sample Year") +
      ylab("November-May Total Rainfall (cm)") +
      ggtitle("Rainfall Estimated at Plots from Rain Gauges")
```
