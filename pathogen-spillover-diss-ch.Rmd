...
oakplots.model.data <- select(oakplots.sub, plotid, sample_year, oak.inf, tot_lfct.log, tot_lfct.log.scl, hrs_abv25ds, hrs_abv25ds.scl, avg_hrs1422_wet_t_t1.log, avg_hrs1422_wet_t_t1.log.scl, umca_density_ha.log, umca_density_ha.log.scl, H.2005, shannons2005.scl, twi15m.log, twi15m.log.scl)

oakplots.model.data <- filter(oakplots.model.data, is.na(H.2005)==F)
...
```{r hours >25 path model no umca density unscaled, echo=F}
model_list2 <- list(
...
```{r hours >25 path model no umca density scaled, echo=F}
...
model_list1 <- list(
      ...
      umca_density <- lmer(umca_density_ha.log ~ twi15m.log + (1|plotid) + (1|sample_year), data = oakplots.model.data),
      ...
      wet_hours_1422 <- lmer(avg_hrs1422_wet_t_t1.log ~ 
                                   ...
                                   twi15m.log + (1|plotid) + (1|sample_year), data = oakplots.model.data),
...
pairs(na.omit(select(oakplots.sub, umca_density_ha.log.scl, hrs_abv25ds.scl, avg_hrs1422_wet_t_t1.log.scl, shannons2005.scl, twi15m.log.scl)),
...
plot(wet_hours_1422@frame$twi15m.log.scl, resid(wet_hours_1422),
     xlab = "bay laurel density",
     ylab = "model residuals",
     main = "Wet-hours model residuals vs. bay laurel density predictor")
```
...
pairs(na.omit(select(oakplots.sub, tot_lfct.log.scl, avg_hrs1422_wet_t_t1.log.scl, shannons2005.scl)),
      lower.panel = panel.cor, diag.panel = panel.hist,
...
raw_coefficients <- sem.coefs(model_list1, oakplots.model.data,
          corr.errors = c("hrs_abv25ds ~~ avg_hrs1422_wet_t_t1.log"))
scaled_coefficients <- sem.coefs(model_list1.scl, oakplots.model.data,
          corr.errors = c("hrs_abv25ds.scl ~~ avg_hrs1422_wet_t_t1.log.scl"))
