...
![Conceptual Path Model](figures/temperature-path-model-updated.png)
...
model_list1_refit <- list(
      ...
      umca_density_refit <- lmer(
            umca_density_ha.log ~ H.2005 + twi15m + (1|plotid) + (1|sample_year), 
            data = oakplots.model.data),
      
      slc_refit <- lmer(
            tot_lfct.log ~ umca_density_ha.log + hrs_abv25ds + avg_hrs1422_wet_t_t1.log + H.2005 + (1|plotid) + (1|sample_year), 
                  data = oakplots.model.data),
      
      wet_hours_1422_refit <- lmer(
            avg_hrs1422_wet_t_t1.log ~ twi15m + (1|plotid) + (1|sample_year), 
                             data = oakplots.model.data),
      
      hrs_abv25_refit <- lmer(
            hrs_abv25ds ~ twi15m + (1|plotid) + (1|sample_year), 
            data = oakplots.model.data),
      
      oak_infection_refit <- glmer(
            oak.inf ~ tot_lfct.log + avg_hrs1422_wet_t_t1.log + H.2005 + (1|plotid) + (1|sample_year), 
            family = binomial(link = "logit"), 
            data = oakplots.model.data)
)
```

```{r path-model-no-lide-plots, echo=F}
...
## Model with all LIDE plots excluded
