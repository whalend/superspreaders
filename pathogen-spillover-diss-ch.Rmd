...
GGally::ggpairs(select(oaks.data, h2005, twi15m, tot_bay, tot_bay.ln, tot_bay.ln1))
...
m_list_update <- list(
            ...
            shannon.mod <- lm(h2005 ~ twi15m, data = filter(oaks.data, year==2006), na.action = na.omit),
      ...
      lfct.mod <- lmer(lfct ~ tot_bay.ln1 + hrs25 + hrs1422 + (1|plotid) + (1|year), data = oaks.data, na.action = na.omit),
...
S.plot <- plot_model(shannon.mod, type = "slope")

# line_color <- "black"
# S.plot$data
# str(S.plot)

(shannon.plot <- ggplot(oaks.data, aes(twi15m, h2005)) +
      geom_point(alpha = .7) +
...
# effect plots
plot_model(lfct.mod, type = "slope") +
      theme_bw() +
      ylab("log (leaf count + 1)") +
      theme(strip.background = element_blank()) +
      NULL

plot_model(lfct.mod, type = "pred", terms = c("hrs25", "tot_bay.ln1")) +
      theme_classic() +
      ylab("log( leaf count + 1 )") +
      xlab("Hours >25 ºC") +
...
cowplot::plot_grid(slc_vs_bay_plot, slc_vs_hrs25_plot, nrow = 2)
...
ggplot(oaks.data, aes(twi15m, hrs25)) +
      geom_point(alpha = .3) +
      geom_smooth(method = "lm", color = "black") +
      xlab("Topographic index") +
      ylab("Number of hours >25 ºC") +
...
cowplot::plot_grid(
      plot_model(inf.mod, type = "est", show.values = T, 
                 title = "Oak infection - est."),
      plot_model(inf.mod, type = "std", show.values = T, 
                 title = "Oak infection std. est."),
      nrow = 2
      )

plot_model(inf.mod, type = "pred", terms = c("lfct")) +
      # geom_point() +
      theme_classic() +
      xlab("log( symptomatic leaf count + 1 )") +
      ylab("Predicted probability for oak infection") +
      ggtitle("") +
      NULL

ggplot(oaks.data, aes(lfct, inf_oak_ct/(inf_oak_ct+uninf_oak_ct))) +
      geom_point() +
...
infmod.resids <- resid(infmod)
Moran.I(infmod.resids, pl.dists.inv)
cgram.inf.resids <- correlog(oaks.data$x, oaks.data$y, infmod.resids, increment = 300, resamp = 99)
...
cgram.lfct <- correlog(oaks.data$x, oaks.data$y, oaks.data$tot_lfct.log, increment = 300, resamp = 99)
plot(cgram.lfct, main = "Leaf count response correlogram")

r.lfctmod <- resid(lfctmod)
