...
summary(na.omit(oakplots.2006))
points <- select(na.omit(oakplots.2006), X, Y)
points <- as.matrix(dist(cbind(points$X, points$Y)))
points.inv <- 1/points
diag(points.inv) <- 0
...
summary(points_rep)
points_rep <- as.matrix(dist(cbind(points_rep$x, points_rep$y)))
points_rep.inv <- 1/points_rep
points_rep.inv[points_rep.inv == "Inf"] <- 0
summary(points_rep.inv)

# Plot variograms
vario.umca_density <- variog(coords = points_rep, 
                     data = na.omit(umca_density@frame$tot_bay.log),
                     uvec = (seq(0, max.dist, length = bins)))
...
vario.slc <- variog(coords = points_rep, 
                     data = na.omit(slc@frame$tot_lfct.log),
                     uvec = (seq(0, max.dist, length = bins)))
...
# par(mfrow = c(1,2))
plot(vario.umca_density.resids, 
     main = "Variogram - UMCA deensity model residuals",
     xlab = "distance (m)")
lines(fit.vario.umca_density.resids)
abline(h = fit.vario.umca_density.resids$nugget, col = "blue")# nugget
abline(h = fit.vario.umca_density.resids$cov.pars[1] + fit.vario.umca_density.resids$nugget, col = "green")# sill
