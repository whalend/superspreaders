...
library(piecewiseSEM); library(dplyr); library(lme4); library(lmerTest)
...
oakplots.sub$tot_bay.scl <- scale(oakplots.sub$tot_bay)
oakplots.sub$tot_bay.log.scl <- scale(oakplots.sub$tot_bay.log)
oakplots.sub$umca_density_ha.scl <- scale(oakplots.sub$umca_density_ha)
oakplots.sub$umca_density_ha.log.scl <- scale(oakplots.sub$umca_density_ha.log)
oakplots.sub$umca_basal_area.scl <- scale(oakplots.sub$umca_basal_area)
oakplots.sub$umca_ba.log.scl <- scale(oakplots.sub$umca_ba.log)
oakplots.sub$tot_lfct.scl <- scale(oakplots.sub$tot_lfct)
oakplots.sub$tot_lfct.log.scl <- scale(oakplots.sub$tot_lfct.log)
# oakplots.sub$avg_lfct.scl <- scale(oakplots.sub$avg_lfct)
oakplots.sub$hrs_abv25ds.scl <- scale(oakplots.sub$hrs_abv25ds)
oakplots.sub$dys_abv25ds.scl <- scale(oakplots.sub$dys_abv25ds)
oakplots.sub$avg_tmax_ds.scl <- scale(oakplots.sub$avg_tmax_ds)
oakplots.sub$avg_hrsblw14_wet_tminus1.scl <- scale(oakplots.sub$avg_hrsblw14_wet_tminus1)
oakplots.sub$hrs1422_wet_tminus1.log.scl <- scale(oakplots.sub$hrs1422_wet_tminus1.log)
# oakplots.sub$avg_hrs1422_wet_t_t1.scl <- scale(oakplots.sub$avg_hrs1422_wet_t_t1)
oakplots.sub$avg_hrs1422_wet_t_t1.log.scl <- scale(oakplots.sub$avg_hrs1422_wet_t_t1.log)

## static variables
oakplots.sub$candens15m.scl <- scale(oakplots.sub$candens15m)
oakplots.sub$shannons2005.scl <- scale(oakplots.sub$H.2005)
oakplots.sub$shannons2014.scl <- scale(oakplots.sub$H.2014)
oakplots.sub$twi15m.scl <- scale(oakplots.sub$twi15m)
oakplots.sub$twi15m.log.scl <- scale(oakplots.sub$twi15m.log)
```
...
Using data on oak infection at the plot-level:
```{r hours >25 path model all oak plots unscaled, echo=F}
model_list1 <- list(
      shannons <- lm(H.2005 ~ twi15m.log, data = filter(oakplots.sub, sample_year == 2005)),
...
model_list1.scl <- list(
      ...
      slc <- lmer(tot_lfct.log.scl ~ tot_bay.log.scl + hrs_abv25ds.scl + avg_hrs1422_wet_t_t1.log.scl + shannons2005.scl + twi15m.log.scl + (1|plotid) + (1|sample_year), data = oakplots.sub),
...
par(mfrow=c(2,2))
plot(shannons, main = "OLS regression of diversity ~ wetness index")
```

```{r bay laurel density residuals plot}
plot(umca_density, main = "LMM of bay laurel density")
```

```{r symptomatic leaf count residuals plot}
plot(slc, main = "LMM of symptomatic leaf count")
```

```{r wet-hours residuals plot}
plot(wet_hours_1422, main = "LMM of avg. # of hours 14-22 C current & prev. year")
```

```{r dry-season hours residuals plot}
plot(hrs_abv25, main = "LMM of # of hours >25 C during prev. dry season")
```

```{r oak infection residuals plot}
plot(oak_infection, main = "Binomial GLMM of oak infection")
```
