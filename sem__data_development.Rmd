...
unique(rs_temps$plotid)# 201 levels
unique(summer_temps$plotid)# 201 levels
unique(rain_df$plotid)# 202 levels
...
# I think the 'facc_ws15m' is the estimate from the 'r.watershed' function in GRASS GIS
...
So now I have a plot-level of UMCA infection levels for each year in terms of symptomatic leaf counts and number of un/infected stems. I want to join this with some of the plot-level environmental data.
...
# NAs in rainfall data are from 2003
filter(umca_plots, sample_year == 2003)
# Only 7 plots w/UMCA sampled in 2003, so removing these records
umca_plots <- filter(umca_plots, sample_year != 2003)
```
...
unique(filter(canopy_cover, year == 2006)$plotid)
...
# rain_total$sample_year <- as.numeric(rain_total$sample_year)
summary(rain_days)
rain_days$plotid <- tolower(rain_days$plotid)
rain_days <- rename(rain_days, sample_year = year)
#rain_days$sample_year <- as.numeric(as.character(rain_days$sample_year))
...
### Save patha analysis data environment
