...
# These were taken from the help pages of the  `pairs` function with minor alterations
...
pairs(subset(data, select = c(lat, Date, DD, Growth, Live)),
...
fit1 <- lme(Date ~ DD + lat, data = data, 
            random = ~1|site/tree, 
            na.action = na.omit)
fit1lme4 <- lmer(Date ~ DD + lat + (1|site) + (1|tree), data = data,
                 na.action = na.omit)
summary(fit1nlme)
summary(fit1lme4)
```
...
fit2 <- lme(Growth ~ Date + lat, data = data, 
            random = ~1|site/tree, 
            na.action = na.omit)
# Independence of Growth and DD conditioned on Date and lat
fit3 <- lme(Growth ~ Date + lat + DD, data = data, 
            random = ~1|site/tree, 
            na.action = na.omit)
```
...
Combine the p-values into the *C*-statistic
```{r calculate C statistic}
ln_pvalues <- log(pvalues)
C_stat <- -2*sum(ln_pvalues)
```
I would like to figure out how to calculate the chi-square cumulative probability based on the degrees of freedom and *C*-statistic, a la this [website](http://stattrek.com/online-calculator/chi-square.aspx)
