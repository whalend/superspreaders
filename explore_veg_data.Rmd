...
alt_sp_live <- filter(alt_sp_dbh, Dead == "no")
alt_sp_dead <- filter(alt_sp_dbh, Dead == "yes")

alt_sp_summ <- left_join(
      # First data frame
      alt_sp_live %>% filter(year != 2012) %>% group_by(PlotID, Species, year) %>% 
      summarise(live_count = length(Species), live_avg_dbh = mean(dbh), live_tot_dbh = sum(dbh)),
      # Second data frame
      alt_sp_dead %>% filter(year != 2012) %>% group_by(PlotID, Species, year) %>% 
      summarise(dead_count = length(Species), dead_avg_dbh = mean(dbh), dead_tot_dbh = sum(dbh))
)

write.csv(alt_sp_summ, "analysis/data/untagged_dbh_summary.csv")
```
