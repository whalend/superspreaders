...
# Spot checking some of these shows that NAs are due to missing data, i.e. they should not be changed to zeroes
...
qusp_stems <- stems %>% 
      select(plotid, year, tag, species, canker, stem_status, infection_confirmed_1plus, infection_confirmed_2plus) %>% 
      filter(species == "quag" | species == "quke" | species == "quch" |
                   species == "quwi", stem_status == "Alive")
...
oak_sod <- left_join(oak_sod, 
                     select(umca_plots, plotid, sample_year, uninfected_bay_ct, infected_bay_ct, tot_bay, tot_lfct), 
