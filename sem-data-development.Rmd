...
length(unique(oak_sod$plotid))# 164 plots remaining


pairs(na.omit(select(oak_sod, c(3,4,7:10), H.2005, H.2014)),
      lower.panel = panel.cor, diag.panel = panel.hist, 
...
rain_lag <- as.tbl(select(rain_total, plotid, sample_year, rain_tot_v))
rain_lag1 <- as.tbl(select(rain_total, plotid, sample_year))
rain_lag1 <- rain_lag1 %>% 
      mutate(sample_year_minus1 = sample_year - 1, 
             sample_year_minus2 = sample_year - 2,
             sample_year_minus3 = sample_year - 3) 
rain_lag1 <- left_join(rain_lag1, rain_lag,
                by = c("plotid", "sample_year_minus1"="sample_year")) %>%
      rename(rain_tot_t_minus1 = rain_tot_v)
rain_lag1 <- left_join(rain_lag1, rain_lag,
                by = c("plotid", "sample_year_minus2"="sample_year")) %>%
      rename(rain_tot_t_minus2 = rain_tot_v)
rain_lag1 <- left_join(rain_lag1, rain_lag,
                by = c("plotid", "sample_year_minus3"="sample_year")) %>%
      rename(rain_tot_t_minus3 = rain_tot_v)
rain_lag1 <- left_join(rain_lag1, rain_lag,
                by = c("plotid", "sample_year"="sample_year"))
rain_lag1 <- select(rain_lag1, plotid, sample_year, starts_with("rain"))

rain_lag1 <- rain_lag1 %>% 
      mutate(avg_rain_t_minus1 = 
                   rowMeans(cbind(rain_tot_v, rain_tot_t_minus1)),
             avg_rain_t_minus2 =
                   rowMeans(cbind(rain_tot_v, rain_tot_t_minus1, rain_tot_t_minus2)),
             avg_rain_t1t2 = rowMeans(cbind(rain_tot_t_minus1, rain_tot_t_minus2)))
summary(rain_lag1)
```
