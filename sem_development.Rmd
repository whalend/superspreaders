...
```
```{r load physical data, echo=FALSE}
...
summary(plots_env)
plots_env <- select(plots_env, -cat)
...
plots_vegtopo <- select(plots_vegtopo, -X, -Y)
```
...
# Rainfall: interpolated rainfall for each season
...
plots_rain <- rename(plots_rain, plotid = PLOT_ID)
```
...
```{r rain wide to long}
library(tidyr)
rain2d <- plots_rain %>% select(plotid, ends_with("2d"))
names(rain2d) <- c("plotid",paste(c(2004:2012,2014), sep = ","))
rain2d <- rain2d %>% gather(year, rain_tot_2d, -plotid)

rain3d <- plots_rain %>% select(plotid, ends_with("3d"))
names(rain3d) <- c("plotid",paste(c(2004:2012,2014), sep = ","))
rain3d <- rain3d %>% gather(year, rain_tot_3d, -plotid)

rain_v <- plots_rain %>% select(plotid, ends_with("v"))
names(rain_v) <- c("plotid",paste(c(2004:2012,2014), sep = ","))
rain_v <- rain_v %>% gather(year, rain_tot_v, -plotid)

rain_data <-  left_join(rain2d, 
                        left_join(rain3d, rain_v, by = c("plotid","year")),
                        by = c("plotid","year"))
summary(rain_data)
rm(rain_v); rm(rain2d); rm(rain3d)
...
summary(plots_env)
str(plots_env)
plots_env <- plots_env %>% rename(plotid = PLOT_ID, easting = X, northing = Y)
plots_env <- select(plots_env, -cat)
...
unique(rain_data$plotid)# 202 levels
tempsid <- as.character(unique(plots_temps$plotid))
rainid <- as.character(unique(rain_data$plotid))
rainid %in% tempsid
rainid[62]# BUSH01 still in the rain data
rain_data <- filter(rain_data, plotid != "BUSH01")
...
qplot(slope15m, flowacc15m, data = filter(plots_env, flowacc15m < 20), 
      main = "Flow Accumulation < 5") +
      geom_text(aes(label = plotid))
qplot(slope15m, flowacc15m, data = filter(plots_env, flowacc15m > 20 & flowacc15m < 1000), main = "Flow Accumulation 5 - 1000") +
      geom_text(aes(label = plotid))
...
live_tagged_dbh <- stems %>% filter(status == "Alive") %>% group_by(plotid, year, species) %>% summarise(live_count = length(species))
unique(live_tagged_dbh$species)
os_richness <- rbind(live_tagged_dbh, select(untagged_dbh, plotid, species, year, live_count))
...
oak_sod <- left_join(oak_sod, select(plots_umca, plotid, year, uninfected_bay_ct, infected_bay_ct, tot_bay, avg_lfct, tot_lfct), 
                     by = c("plotid", "year"))
...
oak_sod <- left_join(oak_sod, plots_temps, 
                     by = c("plotid", "year" = "year"))
oak_sod
